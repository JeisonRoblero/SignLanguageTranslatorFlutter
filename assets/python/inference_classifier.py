...
model_dict = pickle.load(open('assets/python/model.p', 'rb'))
model = model_dict['model']

# Obtiene el número de características que el modelo espera
expected_features = model.n_features_in_

# Carga los datos para ajustar la longitud de lo que detecte
data_dict = pickle.load(open('assets/python/data.pickle', 'rb'))  # Carga los datos y las etiquetas desde el archivo 'data.pickle'.
...
while True:  # Bucle infinito para procesar cada fotograma de video.
    ...
    cv2.putText(frame, 'Presiona "e" para salir',
                    (10, 460), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1,
    ...
    if results.multi_hand_landmarks:  # Si se detectan manos en el fotograma.
        ...
        # Ajusta data_aux a la longitud esperada por el modelo
        if len(data_aux) > expected_features:
            data_aux = data_aux[:expected_features] # Recorta si hay mas elementos de los esperados
        else:
            data_aux += [0] * (expected_features - len(data_aux)) # Rellena con ceros si hay menos elementos de los esperados
